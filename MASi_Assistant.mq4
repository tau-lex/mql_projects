...
//-----------------Global variables-------------------------------------------+
const string    Copyright = "Copyright 2017, Terentyev Aleksey";
        ...
        while( !FileIsEnding( config ) ) {
            ...
            if( StringFind( tmpString, section ) >= 0 ) {
                ...
                if( StringFind( tmpString, "Input=" ) >= 0 ) {
                    ...
                    StrSplit( StringTrimRight( StringSubstr( tmpString, 6 ) ), ';', inputList );
                    tmpString = FileReadString( config );
                }
                if( StringFind( tmpString, "Output=" ) >= 0 ) {
                    StringReplace( tmpString, "\"", "" );
                    StrSplit( StringTrimRight( StringSubstr( tmpString, 7 ) ), ';', outputList );
                    tmpString = FileReadString( config );
                }
            }
        }
...
{
    string symbolsList[];
    int size = SymbolsList( selected, symbolsList );
    if( size >= 1 ) {
